...
from src.utils.constants import NPL_OUTLIER_THRESHOLD
...
class MetricsCalculator:
    ...
    def calculate_all_metrics(dados: Dict[str, Any]) -> FundMetrics:
        ...
        zumbi_ratio = MetricsCalculator.calculate_zumbi_ratio(aging_maior_1080, carteira_total)
    ...
    def add_metrics_to_data(dados: Dict[str, Any]) -> None:
        """
        Calcula e adiciona métricas ao dicionário de dados (in-place).
        ...
        npl_outlier = metrics.npl_ratio > NPL_OUTLIER_THRESHOLD
        dados["NPL_OUTLIER_FLAG"] = npl_outlier
        if npl_outlier:
            logger.warning(
                f"NPL outlier: {metrics.npl_ratio:.2f}% | CNPJ: {dados.get('CNPJ_FUNDO', 'N/A')}"
            )

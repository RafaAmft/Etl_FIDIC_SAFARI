...
from src.utils.constants import CNPJ_INVALIDO_VALUES, CSV_ENCODING
...
class CSVExporter:
    ...
    def __init__(self, output_dir: Optional[Path] = None) -> None:
        ...
        """
        self.output_dir: Path = output_dir or settings.get_output_subdir()
    ...
    def _prepare_dataframe(
        ...
        Returns:
            DataFrame validado, ou None se vazio.
        """
        if not dados:
            return None
        ...
        # Filtrar registros com CNPJ válido
        if "CNPJ_FUNDO" in df.columns:
            df = df[
                df["CNPJ_FUNDO"].notna()
                & ~df["CNPJ_FUNDO"].astype(str).isin(CNPJ_INVALIDO_VALUES)
            ]
    ...
    def _dedup_and_export(
        ...
        Args:
            ...
            dedup_cols: Colunas para deduplicação (opcional).
            sort_cols: Colunas para ordenação (opcional).
            sort_ascending: Direção de ordenação.
    ...
    def export_monitor_completo(
        ...
        Returns:
            Path do arquivo salvo, ou None se sem dados válidos.
        """
        ...
        filepath = self.output_dir / f"fidc_monitor_completo_{ts}.csv"
    ...
    def export_safari_oportunidades(
        ...
        """
        Exporta apenas oportunidades do Safari (score >= score_min).

        Args:
            dados: Lista de dicionários com dados dos fundos.
            score_min: Score mínimo para inclusão.
            timestamp: Timestamp customizado (opcional).

        Returns:
            Path do arquivo salvo, ou None se sem dados.
        """
        df = self._prepare_dataframe(dados, filter_success=True)
        if df is None:
        ...
        ts = timestamp or self._current_timestamp()
        filepath = self.output_dir / f"fidc_safari_oportunidades_{ts}.csv"

        self._dedup_and_export(df, filepath, sort_cols=["SCORE_SAFARI"], sort_ascending=False)

        logger.info(f"Safari oportunidades salvo: {filepath} ({len(df)} linhas)")
        return filepath
    ...
    def export_distressed_npl(
        ...
        Args:
            dados: Lista de dicionários com dados dos fundos.
            npl_min: NPL mínimo para inclusão (%).
            timestamp: Timestamp customizado (opcional).
        ...
        ts = timestamp or self._current_timestamp()
        filepath = self.output_dir / f"fidc_distressed_npl_gt_{int(npl_min)}_{ts}.csv"

        self._dedup_and_export(df, filepath, sort_cols=["NPL_RATIO"], sort_ascending=False)

        logger.info(f"Distressed NPL salvo: {filepath} ({len(df)} linhas)")
        return filepath
